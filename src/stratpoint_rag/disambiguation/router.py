...
_OFF_TOPIC_RESPONSE = "I'm sorry, I can only answer questions about Stratpoint — their services, projects, technologies, and company information. Could you ask something related to Stratpoint?"
...
_active_loops: dict[str, ClarificationLoop] = {}
...
def route(
    ...
    if session_id and session_id in _active_loops:
        loop = _active_loops[session_id]
        if not loop.is_complete():
            slot_query = loop.process_answer(user_input)
            if loop.is_complete():
    ...
    intent_query = classify(user_input)

    if intent_query.confidence < 0.7:
...
def get_active_loop(session_id: str) -> ClarificationLoop | None:
