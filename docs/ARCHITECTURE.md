...
**Related**: `docs/architecture-flow.md` narrates the same system as a runtime request flow with deep-dives on guardrail and disambiguation policy. This document is the file-level map — what each file is and what it depends on. The repo-root SVGs draw the agent turn at three zoom levels — `react-tool-1-overview.svg` (the turn end to end, ~10 boxes), `react-tool-2-loop.svg` (the loop and its six tools), `react-tool-3-chains.svg` (what each tool actually touches: the answer, brief and money chains) — plus `react-tool.svg`, all of it on one page. All four are generated by `util/gen_react_svg.py`, not hand-drawn; lavender means "was in the original diagram", mint green means "added since".
...
| `currency_calculator.py` | Not a directory — the package-root pricing module: the handbook's PHP rates by role, PHP↔USD conversion at a fixed 60:1, the whole-word matcher both it and `pdf_gen/mapping.py` use, and the category costing add-ons |
...
| `uv run python util/gen_react_svg.py` | Standalone script (stdlib only) — regenerates all four repo-root diagrams: `react-tool-1-overview.svg`, `react-tool-2-loop.svg`, `react-tool-3-chains.svg`, and the single-page `react-tool.svg`. They are *generated*, not drawn: the predecessor was a Figma export with every label flattened to a `<path>`, so it could not be corrected as the tool set grew. Edit the script, re-run it, never hand-edit an SVG. |
...
| File | What it does | Depends on |
|---|---|---|
| `currency_calculator.py` | The pricing primitives, shared by `agent/tools.py` and `pdf_gen/mapping.py` and importing nothing first-party: the handbook's PHP rates by role and by licence; `convert_currency()` at a fixed 60 PHP = 1 USD; `calculate_role_rate()` / `lookup_handbook_rate()`; `get_category_costings()`, which appends Cloud / AI / Data / Security / Licence line items the brief's own features trigger; and `word_in()`, the one whole-word matcher the project shares. Rules are load-bearing and each is there because it was measured wrong: matching is whole-token, never substring (`"ai"` sits inside "Email", "Domain" and "Plain" — a plain CRUD brief billed at the senior AI/ML rate, and category costing *added* an AI specialist and a licence nobody asked for) | — |
...
| `react-tool.svg`, `react-tool-1-overview.svg`, `react-tool-2-loop.svg`, `react-tool-3-chains.svg` (repo root, committed) | `util/gen_react_svg.py` — regenerate, never hand-edit | the report deck, at three zoom levels plus the single-page view. Generated because the original was a Figma export with every label flattened to a `<path>`, which made it uncorrectable as the tool set grew |
