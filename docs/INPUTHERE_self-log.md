...
[2026-07-07] Fixed guardrail integration with ReAct agent and rag. The chunking was also reduced to fix for longer sentence questions catering to the `find_resources` tool. The ChatUI has now an implementation of a download pdf.

[2026-07-05] Made an agentic API design spec for the ReAct agent using NVIDIA NIM integrated langchain development. Two tools were made, `search_stratpoint` and `find_resource`. The `find_resource` tool finds files embedded in the RAG corpus and returns them to the agent for the user. This does not however cover cases where files are removed from the actual links themselves. 
To implement:
